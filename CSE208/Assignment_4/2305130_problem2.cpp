...
int main()
{
    ...
    int testNo = 5;
    string inFile = "Test_Cases/Problem 2/Inputs/" + to_string(testNo) + ".txt";
    string outFile = "Test_Cases/Problem 2/Outputs/" + to_string(testNo) + ".txt";
    ...
    int s = n-2;
    int t = n-1;
    for (int i = 0; i < k; i++) adjMat[s][i] = 1;
    for (int i = k; i < n-2; i++) adjMat[i][t] = 1;
    ...
    for (auto e : res.flow)
    {
        if (e.u < s && e.v < s && e.w > 0) cout << e.u << ' ' << e.v << '\n';
    }
